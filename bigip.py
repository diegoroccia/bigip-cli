...
def get_members(ctx, args, incomplete):
    try:
        ...
        return [
            member.name
            for member in mgmt.tm.ltm.pools.pool.load(
                name=pool, partition="Common"
            ).members_s.get_collection()
...
def cli(ctx, partition, pool, verbose):
    ...
    ctx.obj["mgmt"] = mgmt
    if pool:
        ctx.obj["pool"] = ctx.obj["mgmt"].tm.ltm.pools.pool.load(
            name=pool, partition="Common"
        )
...
@click.argument(
    "member_name", type=click.STRING, default="", autocompletion=get_members
)
...
def members_stats(ctx, member_name, show_all):
    ...
    for member in ctx.obj["pool"].members_s.get_collection():
        if ((member_name in member.name) or (member_name.lower() == "all")) and (
            member.state == "up" or show_all
        ):
            values = {
                metric.split(".")[1]: value.get("value", 0)
...
def members_watch(ctx, member_name, counter_name):
    for member in ctx.obj["pool"].members_s.get_collection():
        if (
            (member_name in member.name) or (member_name.lower() == "all")
        ) and member.state == "up":
            if counter_name:
...
def members_list(ctx):
    for member in ctx.obj["pool"].members_s.get_collection():
        click.secho(
            "{name} [{state}]".format(**member.to_dict()),
